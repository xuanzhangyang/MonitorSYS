...
CButtonST::CButtonST()
{
	...
	m_bAlwaysTrack = TRUE;
	m_btfalseColor = ::GetSysColor(COLOR_3DSHADOW);
...
void CButtonST::setfalseColor(COLORREF btfalseColor)
{
	this->m_btfalseColor = btfalseColor;
}
...
// 			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
			captionRect.OffsetRect(-1, -1);
			pDC->SetTextColor(m_btfalseColor);
			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
		} // if
		else
		{
