...
//--自己加入的函数--用来删除所有的标示
//加入时间2012-3-23，尘中远
//调用前最好m_ChartCtrl.EnableRefresh(FALSE);//暂停刷新
template<class T> 
void CChartSerieBase<T>::ClearAllBalloonLabel()
{
	TLabelMap::iterator iter = m_mapLabels.begin();
	for (iter; iter!=m_mapLabels.end(); iter++)
	{
		iter->second->SetLabelText(_T(""));
	}
}
// 
// //--自己加入的函数--用来删除所有的标示
// //加入时间2012-3-23，尘中远
// template<class T> 
// void CChartSerieBase<T>::DeleteOneBalloonLabel(unsigned uPointIndex)
// {
// 	ASSERT(uPointIndex<GetPointsCount());
// 
// 	TLabelMap::iterator iter = m_mapLabels.find(uPointIndex);
